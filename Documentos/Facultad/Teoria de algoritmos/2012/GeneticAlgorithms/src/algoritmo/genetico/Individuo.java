...
public class Individuo implements Comparable<Individuo> {
	...
	public Individuo(byte gen) {
		this.gen = gen;
		genBinario = new StringBuilder(Integer.toBinaryString(gen));
		
		// Completamos el gen con 7 bits para evitar problemas
		if(genBinario.length() < 7) {
			genBinario = new StringBuilder("0" + "0000000".substring(0, 7 - genBinario.length()) + Integer.toBinaryString(gen));
		}
	}
	...
	public Individuo(byte gen, StringBuilder genBinario){
		...
		// Completamos el gen con 7 bits para evitar problemas
		if(genBinario.length() < 7) {
			genBinario = new StringBuilder("0000000".substring(0, 7 - genBinario.length()) + Integer.toBinaryString(gen));
		}
	}
	...
	public void calcularAptitud(FuncionDeAptitud funcionDeAptitud){
		aptitud = funcionDeAptitud.evaluar(this);		
	}
	...
	public Individuo mutar(double pctMutacion){
		...
		byte genMutado = (byte) Integer.parseInt(genBinarioMutado.toString(), 2);
